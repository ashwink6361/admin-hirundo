...
gulp.task('inject', ['scripts', 'styles', 'injectAuth', 'injectResetPath', 'inject404', 'copyVendorImages'], function () {
...
gulp.task('injectAuth', ['stylesAuth'], function () {
  return injectAlone({
    ...
    paths: [path.join(conf.paths.src, '/auth.html'), path.join(conf.paths.src, '/invoice.html'), path.join(conf.paths.src, '/reg.html')]
  })
});

gulp.task('injectResetPath', ['stylesAuth'], function () {
  return injectAlone({
    css: [path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'), path.join(conf.paths.tmp, '/serve/app/auth.css')],
    paths: [path.join(conf.paths.src, '/user-reset-password.html')]
  })
});
